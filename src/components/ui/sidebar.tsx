...
function SidebarTrigger({
  ...
  return (
    <Button
      ...
      size="icon"
      className={cn(
        'bg-sidebar-accent hover:bg-sidebar-accent/75 size-10 rounded-4xl hover:size-11 active:size-9',
        'fixed top-1/2 z-10 -translate-y-1/2 transition-all duration-300 ease-in-out',
        state === 'expanded'
          ? 'left-3 md:left-[var(--sidebar-width)] md:-translate-x-1/2'
          : 'left-3',
        className
      )}
      onClick={(event) => {
...
const sidebarMenuButtonVariants = cva(
  'peer/menu-button flex w-full items-center gap-3 overflow-hidden rounded-md p-2 text-left outline-hidden ring-sidebar-ring transition-[width,height,padding] hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 active:bg-sidebar-accent active:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-50 group-has-data-[sidebar=menu-action]/menu-item:pr-8 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-[active=true]:bg-sidebar-accent data-[active=true]:font-medium data-[active=true]:text-sidebar-accent-foreground data-[state=open]:hover:bg-sidebar-accent data-[state=open]:hover:text-sidebar-accent-foreground group-data-[collapsible=icon]:size-8! group-data-[collapsible=icon]:p-2! [&>span:last-child]:truncate [&>svg]:size-5 [&>svg]:shrink-0',
  {
    variants: {
      ...
      size: {
        default: 'h-14 text-base', // Tinggi tombol & ukuran teks diubah
        sm: 'h-7 text-xs',
